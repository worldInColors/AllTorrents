export function detectSeriesType(title) {
  ...
  const moviePatterns = [
    /\bmovie\b/i,
    /\bfilm\b/i,
    /\bova\b(?!\s*s)/i, // OVA but not "OVAs" (plural might be series)
    /\bspecial\b/i,
  ];
  ...
  if (singleEpisodePatterns.some(pattern => pattern.test(title))) {
    ...
    const hasBatchIndicators = /\bbatch\b|complete|season.*\d+|\d+\s*-\s*\d+(?!\s*\[)/i.test(title);
  ...
  // Check for movies
  if (moviePatterns.some(pattern => pattern.test(title))) {
    return 'movie';
  }
