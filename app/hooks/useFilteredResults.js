...
import {detectSeriesType} from "../utils/detectSeriesType";
...
export  function useFilteredResults({
  ...
  const filteredResults = useMemo(() => {
    ...
    if (sourceFilter && sourceFilter !== "all") {
      filtered = filtered.filter((torrent) => torrent.Tracker === sourceFilter);
    }
    ...
    switch (sortBy) {
      case "seeds":
        filtered.sort((a, b) =>
          sortOrder === "asc" ? a.Seeders - b.Seeders : b.Seeders - a.Seeders
        );
        break;
      case "peers":
        filtered.sort((a, b) =>
    ...
    return filtered;
  }, [searchData?.data?.Results, sortBy, sortOrder, sourceFilter, seriesTypeFilter]);
